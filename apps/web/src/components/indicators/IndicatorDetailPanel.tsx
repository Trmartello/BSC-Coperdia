...
// Dias/Índice grandes abreviam (k/M) para não estourar o layout.
...
// % nunca abrevia ("442,7k%" confunde escala com unidade): valores grandes saem
// agrupados e sem decimais (142731.43 → "142.731%").
function fmtPct(v: number, decimals: number): string {
  if (Math.abs(v) < 1000) return `${v.toFixed(decimals)}%`;
  return `${new Intl.NumberFormat('pt-BR', { maximumFractionDigits: 0 }).format(v)}%`;
}

function fmtBar(v: number | null | undefined, unit: string, decimals = 2): string {
  if (v == null) return '';
  if (unit === 'PERCENTAGE') return fmtPct(v, decimals);
